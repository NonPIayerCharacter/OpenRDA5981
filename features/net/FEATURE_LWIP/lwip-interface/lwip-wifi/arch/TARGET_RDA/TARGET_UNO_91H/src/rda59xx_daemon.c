...
//debug info
r_u32 rda_wland_dbg_level = WLAND_DBG_LEVEL;
r_u32 rda_wpa_dbg_level = WPA_DBG_LEVEL;
r_u32 rda_maclib_dbg_level = WLANDLIB_DBG_LEVEL;
r_u32 rda_wland_dump = WLAND_DBG_DUMP;
r_u32 rda_wpa_dump = WPA_DBG_DUMP;
r_u32 rda_hut_dump = HUT_DBG_DUMP;
...
r_u32 wifi_state = EVENT_STA_DISCONNECTTED;
r_u32 wifi_state_ap = EVENT_AP_STOPED;
...
r_s32 rda59xx_send_wland_msg(rda_msg *msg, r_u32 wait_time)
{
    ...
    res = rda_mail_put(wland_queue, (r_void*)msg, RDA_WAIT_FOREVER);
...
r_s32 rda59xx_send_daemon_msg(rda_msg *msg, r_u32 wait_time)
{
    r_s32 res;
    r_void *msg_sem = NULL;
    ...
    msg_sem = rda_sem_create(0);
    msg->arg3 = (r_u32)msg_sem;
    res = rda_mail_put(daemon_queue, (r_void*)msg, 1000);
    res = rda_sem_wait(msg_sem, wait_time);
    res = rda_sem_delete(msg_sem);
...
static void rda_get_macaddr_from_flash(unsigned char *macaddr)
{
    ...
    if ((ret!=0) || !mac_is_valid(rda_mac_addr)) {
...
r_u32 rda59xx_get_wifi_state()
{  
    return wifi_state;
}
r_u32 rda59xx_get_wifi_state_ap()
{  
...
static r_s32 rda59xx_sta_connect_internal(rda59xx_sta_info *sta_info)
{
    ...
    if(index == R_ERR) {
		wifi_state = EVENT_STA_NOFOUND_AP;
    ...
    while(1){
        ...
        msg.arg2 = (r_u32)&ap;
        res = rda_mail_put(wland_queue, (r_void*)&msg, 2000);
        ...
        WIFISTACK_PRINT("set_addr,ip=%s, netmask=%s, gw=%s\r\n",(char*)&(r_sta_info.ip),(char*)&(r_sta_info.netmask), (char*)&(r_sta_info.gateway)); 
        if (r_sta_info.dhcp) {
        ...
        if (!rda59xx_get_netif_ip(&lwip_sta_netif)) {
            res_t = sys_arch_sem_wait(&lwip_sta_netif_has_addr, 20000);//10000);
            if (res_t == SYS_ARCH_TIMEOUT) {
...
reconn:   
        if(res == R_NOERR){
            wifi_state = EVENT_STA_CONNECTTED;
...
static r_s32 rda59xx_ap_enable_internal(rda59xx_ap_info *ap_info)
{
    ...
    rda_mail_put(wland_queue, (r_void*)&msg, RDA_WAIT_FOREVER);
    ...
    if (!netif_is_link_up(&lwip_ap_netif)) {
        ...
        if (res == SYS_ARCH_TIMEOUT) {
            ...
            wifi_state_ap = EVENT_AP_START_FAIL;
    ...
    wifi_state_ap = EVENT_AP_STARTED;
...
r_s32 rda59xx_ap_disable()
{
    ...
    //rda59xx_send_daemon_msg(&msg, RDA_WAIT_FOREVER);
    rda_mail_put(daemon_queue, (r_void*)&msg, RDA_WAIT_FOREVER);
    return 0;        
}
...
static r_void rda59xx_daemon(r_void *arg)
{
    ...
    while(1){
        rda_mail_get(daemon_queue, (r_void*)&msg, RDA_WAIT_FOREVER);
...
// sta.channel must be 0,so the charge always be true
/*              if((module_state & STATE_AP)&&(r_ap_info.channel != ((*(rda59xx_sta_info*)msg.arg1)).channel)){//add ap.channel/sta.channel
...
*/
            ...
            case DAEMON_AP_ENABLE:
                WIFISTACK_PRINT("DAEMON_AP_ENABLE!res=%d,module=0x%0x\r\n",res,module_state);
                ...
                module_state |= STATE_AP; 
                rda_sem_release((r_void *)msg.arg3);
                WIFISTACK_PRINT("DAEMON_AP_ENABLE,rda_sem_release,RES=%d,module_state=0x%0x\r\n",res,module_state);
                break;
            case DAEMON_AP_DISABLE:
                WIFISTACK_PRINT("DAEMON_AP_DISABLE!module_state=0x%0x\r\n",module_state);
                if(!(module_state & STATE_AP)){
...
r_s32 rda59xx_wifi_init()
{
    ...
    if(init_flag == 1)
        return R_NOERR;
    daemon_queue = rda_mail_create(DAEMON_MAILQ_SIZE, sizeof(r_u32)*4);
    ...
    if(thread_init_flag == 0){
        rda_thread_new("maclib_thread", maclib_task, NULL, RDA_THREAD_STACKSIZE*8, RDA_PRIORITY);
        rda_thread_new("wland_thread", rda59xx_wland_task, NULL, RDA_THREAD_STACKSIZE*8, RDA_PRIORITY);
        rda_thread_new("daemon_thread", rda59xx_daemon, NULL, RDA_THREAD_STACKSIZE*5, RDA_PRIORITY);
