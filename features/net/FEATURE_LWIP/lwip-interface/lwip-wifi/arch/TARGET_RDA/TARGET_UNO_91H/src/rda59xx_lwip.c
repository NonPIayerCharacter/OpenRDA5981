...
r_void rda59xx_add_dns_addr(struct netif *netif)
{
...
char *rda59xx_get_ip_address(char *buf, size_t buflen, size_t module)
{
    const ip4_addr_t *addr = 0;
    if(module == 0)
        addr = rda59xx_get_netif_ip(&lwip_sta_netif);
    else
        addr = rda59xx_get_netif_ip(&lwip_ap_netif);
    if (!addr) {
        return NULL;
    }
    if (IP_IS_V4(addr)) {
...
const char *rda59xx_get_netmask(char *buf, size_t buflen,size_t module)
{
    const ip4_addr_t *addr = 0;
    if(module == 0)
        addr = netif_ip4_netmask(&lwip_sta_netif);
    else
        addr = netif_ip4_netmask(&lwip_ap_netif);
...
char *rda59xx_get_gateway_old(char *buf, size_t buflen)
{
    const ip4_addr_t *addr = netif_ip4_gw(&lwip_sta_netif);
    if (!ip4_addr_isany(addr)) {
        return ip4addr_ntoa_r(addr, buf, buflen);
    } else {
        return NULL;
...
char *rda59xx_get_gateway(char *buf, size_t buflen, size_t module)
{
    const ip4_addr_t *addr = 0;
    if(module == 0)
        addr = netif_ip4_gw(&lwip_sta_netif);
    else
        addr = netif_ip4_gw(&lwip_ap_netif);
    if (!ip4_addr_isany(addr)) {
        return ip4addr_ntoa_r(addr, buf, buflen);
    } else {
        return NULL;
