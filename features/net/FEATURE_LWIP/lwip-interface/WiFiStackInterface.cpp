...
int WiFiStackInterface::scan(const char *ssid, const unsigned char channel)
{
	...
	r_scan_info.channel = channel;

	return mbed_lwip_scan_inf(&r_scan_info);
}

int WiFiStackInterface::scan_result(rda59xx_scan_result *bss_list, unsigned int num)
{
    if(bss_list == NULL)
        return 0;
    return mbed_lwip_get_scan_result(bss_list, num);
}

int WiFiStackInterface::ap_join_info(rda59xx_apsta_info_t *sta_list, unsigned int num)
{
    if(sta_list == NULL)
        return 0;
    return mbed_lwip_get_ap_join_info(sta_list, num);
}


void WiFiStackInterface::set_msg_queue(void *queue)
{
    //rda59xx_set_main_queue(queue);
    return;
}
...
{
    //rda59xx_set_sta_listen_interval(interval);
}
...
int WiFiStackInterface::connect(const char *ssid, const char *pass, const char *bssid, nsapi_security_t security, uint8_t channel)
{
	...
	WIFISTACK_PRINT("WIFIStack connect start!\r\n");
    //mbed_lwip_init(NULL);
    ...
    ret = mbed_lwip_bringup(ssid, pass, bssid, _dhcp,
...
int WiFiStackInterface::start_ap(const char *ssid, const char *pass, int channel, char mode)
{
	if(_ip_address_ap[0] == 0){
        ...
        return mbed_lwip_startap(ssid, pass, mbed_ip_addr, mbed_msk_addr, mbed_gw_addr, mbed_dhcp_start, mbed_dhcp_end, channel, mode);
    }else{
        ...
        return mbed_lwip_startap(ssid, pass, (const char *)&ip, (const char *)&mask, (const char *)&gw, (const char *)&dhcps, (const char *)&dhcpe, channel, mode);
    }
}
...
const char *WiFiStackInterface::get_gateway()
{
    if (mbed_lwip_get_gateway(_gateway, sizeof _gateway)) {
        WIFISTACK_PRINT("gateway_addr=%s\r\n",_gateway);
        return _gateway;
    }
