...
extern unsigned int rda_wland_dump;
extern unsigned int rda_wland_dbg_level;
extern unsigned int rda_wpa_dump;
extern unsigned int rda_wpa_dbg_level;
...
void do_recv_thread(void *argument)
{
    ...
    while(1){
        ...
        if(trans_index == 0xff){
            ...
            RESP("+trans_index,IPD=%d,%d,%s,%d,", index, size, rda_socket[index].SERVER_ADDR, rda_socket[index].SERVER_PORT);
            RESP("size=%d,recv=%c\r\n",size,recv_buf[0]);
        }else{
            RESP("+not___trans,IPD=%d,%d,%s,%d,", index, size, rda_socket[index].SERVER_ADDR, rda_socket[index].SERVER_PORT);
            RESP("size=%d,recv=%c\r\n",size,recv_buf[0]);
            continue;
        }
    }
...
int do_nstart(cmd_tbl_t *cmd, int argc, char *argv[])
{
    ...
    if((rda59xx_get_wifi_state() != EVENT_STA_GOT_IP) && (rda59xx_get_wifi_state_ap() != EVENT_AP_STARTED)){
        RESP_ERROR(ERROR_FAILE);
        return 0;
    }
...
{
    int cnt=0, num=0, i;
    rda59xx_scan_result *bss_list = NULL;
    
    cnt = wifi.scan(NULL, 0);

    if(*argv[1] == '?'){
        bss_list = (rda59xx_scan_result *)malloc(cnt * sizeof(rda59xx_scan_result));
        if(bss_list == NULL){
            RESP_ERROR(ERROR_FAILE);
            return 0;
        }
        memset(bss_list, 0, cnt * sizeof(rda59xx_scan_result));
...
//    const char *ip_addr, *mac_addr;
    ...
    if (ret) {
        RDA_AT_PRINT("airkiss fail, ret: %d \r\n", ret);
        RESP_ERROR(ERROR_FAILE);
        return 0;
    } else {
        RDA_AT_PRINT("airkiss success, ssid = \"%s\", pw = \"%s\" \r\n", ssid, pw);
    }
...
int do_dbg( cmd_tbl_t *cmd, int argc, char *argv[])
{
    ...
    if(!strcmp(argv[1], "DRV")){
        rda_wland_dbg_level = arg;
    }else if(!strcmp(argv[1], "WPA")){
        rda_wpa_dbg_level = arg;
...
int do_wsmac( cmd_tbl_t *cmd, int argc, char *argv[])
{
    ...
    if(*argv[1] == '?'){
        rda59xx_get_macaddr((unsigned char *)mac, 0);
...
int do_wamac( cmd_tbl_t *cmd, int argc, char *argv[])
{
    ...
    if(*argv[1] == '?'){
        rda59xx_get_macaddr((unsigned char *)mac, 1);
        RESP_OK_EQU("%02x:%02x:%02x:%02x:%02x:%02x\r\n",
            mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
        return 0;
    }
...
{
    int cnt = 5, num, i;
    rda59xx_apsta_info_t * sta_list;
    if(rda59xx_get_wifi_state_ap() != EVENT_AP_STARTED){
        RESP_ERROR(ERROR_FAILE);
        return 0;
    }
    sta_list = (rda59xx_apsta_info_t *)malloc(5 * sizeof(rda59xx_apsta_info_t));
    memset(sta_list, 0, cnt * sizeof(rda59xx_apsta_info_t));
...
int do_wanet(cmd_tbl_t *cmd, int argc, char *argv[])
{
    ...
    if(*argv[1] == '?'){
        ...
        if((rda59xx_get_wifi_state_ap() == EVENT_AP_STARTED) || (ap_net_flag == 1)){
