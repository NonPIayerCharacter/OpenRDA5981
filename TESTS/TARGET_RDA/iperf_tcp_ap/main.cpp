...
int do_sstart(cmd_tbl_t *cmd, int argc, char *argv[]) {
    if((rda59xx_get_wifi_state() != EVENT_STA_GOT_IP) && (rda59xx_get_wifi_state_ap() != EVENT_AP_STARTED)){
        printf("iperf_server,Please connect ap or start ap!\r\n");
        RESP_ERROR(ERROR_ABORT);
        return 0;
    }
...
int do_cstart(cmd_tbl_t *cmd, int argc, char *argv[])
{
    ...
    if((rda59xx_get_wifi_state() != EVENT_STA_GOT_IP) && (rda59xx_get_wifi_state_ap() != EVENT_AP_STARTED)){
        printf("iperf_client,Please connect ap or start ap!\r\n");
        RESP_ERROR(ERROR_ABORT);
        return 0;
    }
